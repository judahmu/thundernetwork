...
public class Tools {
    ...
    public static long getCoinValueFromInput(Peer peer, List<TransactionInput> input) throws InterruptedException, ExecutionException,
            TimeoutException {
    ...
    public static TransactionWrapper getChannelRefundTransaction(Channel channel) throws SQLException, AddressFormatException {
        ...
        refundTransaction.addOutput(Coin.valueOf(channel.getInitialAmountServer()), new Address(Constants.getNetwork(), channel
        ...
        Sha256Hash sighash = refundTransaction.hashForSignature(0, channel.getOpeningTx().getOutput(0).getScriptPubKey(), SigHash.ALL,
                false);
    ...
    public static boolean checkTransactionLockTime(Transaction transaction, int locktime) {
        if (Math.abs(transaction.getLockTime() - locktime) > 5 * 60) {
            System.out.println("Locktime not correct. Should be: " + locktime + " Is: " + transaction.getLockTime() + " Diff: " + Math.abs(
                    transaction.getLockTime() - locktime));
            return false;
        }
    ...
    /** With reference to
     * http://bitcoin.stackexchange.com/questions/1195/how-to-calculate-
