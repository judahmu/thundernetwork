...
public class ScriptTools {
    ...
    /** Payment outputscript has different forms, depending whether it's
     * serverside or clientside and also depending in which direction the
     * payment goes:
     *
     * <pre>
    ...
    */
    ...
    public static boolean checkRevokeScript(TransactionOutput output, Channel channel, KeyWrapper keyList, boolean serverSide)
            throws Exception {

        /** Revoke Script should be a 2-of-2 multisig with a pubkey of
         * client/server and a tempkey of server/client.. */
    ...
    public static Script getSettlementScriptSig(Channel channel, ECDSASignature signatureClient, ECDSASignature signatureServer,
            String secret, boolean serverSide, boolean paymentToServer) throws Exception {
        ScriptBuilder builder = new ScriptBuilder();
    ...
    public static Script getRefundScriptSig(Channel channel, ECDSASignature signatureClient, ECDSASignature signatureServer,
            boolean serverSide, boolean paymentToServer) throws Exception {
        ScriptBuilder builder = new ScriptBuilder();
    ...
    public static boolean checkPaymentScript(TransactionOutput output, Channel channel, KeyWrapper keyList, String R, boolean serverSide,
            boolean paymentToServer) throws Exception {
    ...
    public static void checkTransaction(Transaction transaction, int inputIndex, Sha256Hash hash, long amount, String payoutAddress,
            int locktime) throws Exception {
        ...
        if (transaction.getOutput(0).getValue().value != amount)
            throw new Exception("Refund Output value not correct. Should be: " + amount + " Is: " + transaction.getOutput(0)
                    .getValue().value);
    ...
    public static boolean isPaymentToServer(TransactionOutput output) {
        /** Hack, that is likely to not work anymore in the future to determine
         * paymentToServer property. However, if this does not work anymore,
         * there is probably no reason to treat these differently. */
        return output.getScriptPubKey().getChunks().get(8).isOpCode();
    }

    public static ArrayList<String> getPubKeysOfChannel(Channel channel, boolean serverSide, boolean temporary) throws ScriptException,
            SQLException {
    ...
    public static Script getPaymentScript(Channel channel, KeyWrapper keyList, String R, boolean serverSide, boolean paymentToServer)
            throws Exception {
        ScriptBuilder builder = new ScriptBuilder();
