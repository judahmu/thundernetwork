...
public class UpdateChannelHandlerThree {
	...
	public UpdateChannelRequestThree request() throws Exception {
		...
		channelTransaction.addOutput(Coin.valueOf(clientTransaction.getOutput(1).getValue().value + Tools
				.getTransactionFees(Constants.SIZE_REVOKE_TX)), ScriptTools.getRevokeScript(keyWrapper, channel,
						Constants.SERVERSIDE));
