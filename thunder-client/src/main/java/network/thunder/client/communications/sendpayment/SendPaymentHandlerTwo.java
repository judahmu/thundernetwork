...
public class SendPaymentHandlerTwo {
	...
	public void evaluateResponse(SendPaymentResponseTwo m) throws Exception {
		/* We got a 3 lists back with refunds, settlements and additional tx
		 * for each payment. Make sure all of those are correct and spends the
		 * corresponding output of our channel.
		 *
		 * Also check the revoke tx that we got, whether it spends our change
		 * correctly. */
		...
		for (int i = 0; i < paymentList.size(); ++i) {
			...
			if (payment.paymentToServer) {
				if (!refund.getOutput(0).getAddressFromP2PKHScript(Constants.getNetwork()).toString().equals(channel
						.getChangeAddressClient()))
					throw new Exception("Refund Output Address is not correct");
				if (!additional.getOutput(0).getAddressFromP2PKHScript(Constants.getNetwork()).toString().equals(channel
						.getChangeAddressClient()))
					throw new Exception("Settlement Output Address is not correct");
				if (!settlement.getOutput(0).getAddressFromP2PKHScript(Constants.getNetwork()).toString().equals(channel
						.getChangeAddressServer()))
			...
			if (payment.paymentToServer) {
				...
				/* TODO: Write a helper to check the settlement against the
				 * temporary key from the server as we cannot use
				 * correctlySpends without R */
			}
		...
		/* Check the revoke payment we got from the client. This payment shall
		 * be timelocked up until the channel ends.
		 *
		...
		if ((channelTransaction.getOutput(0).getValue().value - revoke.getOutput(0).getValue().value) != Tools
				.getTransactionFees(Constants.SIZE_REVOKE_TX))
		...
		/* Validation complete, all transactions we got from the server are
		 * correct.. */
