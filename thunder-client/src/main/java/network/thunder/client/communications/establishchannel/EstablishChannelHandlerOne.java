...
public class EstablishChannelHandlerOne {
	...
	public EstablishChannelRequestOne request() throws Exception {
		...
		/** Rules for the channel: (1) Total value < MAX_CHANNEL_VALUE (2)
		 * serverShare < MAX_SERVER_SHARE (3) Total value > MIN_CHANNEL_VALUE
	...
	public void evaluateResponse(EstablishChannelResponseOne m) throws Exception {
		 ...
		 *
		 * For simplicity, the output for the channel should be the first of the
		 * TX.. Also the client-key is the first in the multisig */
		...
		if (!(transactionFromServer.getOutput(0).getValue().value == (channel.getAmountClient()
				+ channel.getAmountServer())))
			throw new Exception("Channel output does not contain the proper value. Got: "
					+ transactionFromServer.getOutput(0).getValue().value + " Should be: "
