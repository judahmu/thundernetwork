...
public class ClientTools {
	...
	public static Channel createChannel(Connection conn, Wallet wallet, PeerGroup peerGroup,
			ArrayList<Output> outputArrayListlist, long clientAmount, long serverAmount, int timeInDays)
					throws Exception {
		...
		Message requestWrapper = new Message(request, Type.ESTABLISH_CHANNEL_ONE_REQUEST,
				channel.getClientKeyOnClient(), channel.getTimestampOpen());
		...
		EstablishChannelResponseOne responseMessage = new Gson().fromJson(responseOne.data,
				EstablishChannelResponseOne.class);
		...
		EstablishChannelResponseTwo responseMessage2 = new Gson().fromJson(responseTwo.data,
				EstablishChannelResponseTwo.class);
		...
		EstablishChannelRequestThree responseMessage3 = new Gson().fromJson(responseThree.data,
				EstablishChannelRequestThree.class);
	...
	public static Channel makePayment(Connection conn, Channel channel, Payment payment) throws Exception {
		...
		/**
		 * Make sure both sides have sufficient amount of keys before starting a
		 * payment..
		 */
		...
		SendPaymentResponseThree responseMessage3 = new Gson().fromJson(responseThree.data,
				SendPaymentResponseThree.class);
		requestThree.evaluateResponse(responseMessage3);
		conn.commit();
		...
		/**
		 * TODO: The swapping of the transaction ids is probably not very
		 * efficient, as there are MySQL solutions to do it directly.
		 * http://stackoverflow.com/questions/824936/mysql-performance-delete-or
		 * -update
		 */
		...
		try {
			...
			SendPaymentResponseFour responseMessage4 = new Gson().fromJson(responseFour.data,
					SendPaymentResponseFour.class);
	...
	public static void requestKeys(Connection conn, Channel channel, int amountClient, int amountServer)
			throws Exception {
		/**
		 * First request..
		 */
	...
	public static Channel updateChannel(Connection conn, Channel channel, boolean force) throws Exception {
		...
		String response = HTTPS.postToApi(requestWrapper);
		Message responseOne = new Message(response, conn);
		UpdateChannelResponseTwo responseMessage2 = new Gson().fromJson(responseOne.data,
				UpdateChannelResponseTwo.class);
		...
		UpdateChannelResponseThree responseMessage3 = new Gson().fromJson(responseOne.data,
				UpdateChannelResponseThree.class);
		...
		response = HTTPS.postToApi(requestWrapper);
		responseOne = new Message(response, conn);
		UpdateChannelResponseFour responseMessage4 = new Gson().fromJson(responseOne.data,
				UpdateChannelResponseFour.class);
		...
		response = HTTPS.postToApi(requestWrapper);
		responseOne = new Message(response, conn);
		UpdateChannelResponseFive responseMessage5 = new Gson().fromJson(responseOne.data,
				UpdateChannelResponseFive.class);
		requestFive.evaluate(responseMessage5);

		ThunderContext.instance.progressUpdated(6, 10);

		/**
		 * Update complete. We should update our database accordingly: - Change
		 * the amount values in the database - Swap the TXIDs of all payments
		 * and of the channel
		 */
		...
		/**
		 * Find out which payments got removed from the channel with this update
		 * and add these amounts.
		 */
