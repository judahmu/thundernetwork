...
public class Payment {
    ...
    */
    ...
    public Payment(int channelIdSender, int channelIdReceiver, long amount) throws UnsupportedEncodingException,
    ...
    public void updateTransactionsToDatabase(Connection conn) throws SQLException {
        if (settlementTxSenderChanged) {
            settlementTxSenderID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    settlementTxSender, channelIdSender, settlementTxSenderID));
        }
        if (settlementTxReceiverChanged) {
            settlementTxReceiverID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    settlementTxReceiver, channelIdReceiver, settlementTxReceiverID));
        }
        if (refundTxSenderChanged) {
            refundTxSenderID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    refundTxSender, channelIdSender, refundTxSenderID));
        }
        if (refundTxReceiverChanged) {
            refundTxReceiverID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    refundTxReceiver, channelIdReceiver, refundTxReceiverID));
        }
        if (addTxSenderChanged) {
            addTxSenderID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    addTxSender, channelIdSender, addTxSenderID));
        }
        if (addTxReceiverChanged) {
            addTxReceiverID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    addTxReceiver, channelIdReceiver, addTxReceiverID));
        }
        if (settlementTxSenderTempChanged) {
            settlementTxSenderTempID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    settlementTxSenderTemp, channelIdSender, settlementTxSenderTempID));
        }
        if (settlementTxReceiverTempChanged) {
            settlementTxReceiverTempID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    settlementTxReceiverTemp, channelIdReceiver, settlementTxReceiverTempID));
        }
        if (refundTxSenderTempChanged) {
            refundTxSenderTempID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    refundTxSenderTemp, channelIdSender, refundTxSenderTempID));
        }
        if (refundTxReceiverTempChanged) {
            refundTxReceiverTempID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    refundTxReceiverTemp, channelIdReceiver, refundTxReceiverTempID));
        }
        if (addTxSenderTempChanged) {
            addTxSenderTempID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    addTxSenderTemp, channelIdSender, addTxSenderTempID));
        }
        if (addTxReceiverTempChanged) {
            addTxReceiverTempID = MySQLConnection.addOrUpdateTransaction(conn, new TransactionWrapper(
                    addTxReceiverTemp, channelIdReceiver, addTxReceiverTempID));
        }
