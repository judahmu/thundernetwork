...
public class TransactionTimerTask extends TimerTask {
    ...
    public TransactionTimerTask(Timer timer, Connection conn, PeerGroup peerGroup, Transaction transaction, String secretHash,
            int channelId, boolean serverIssuedTransaction, TransactionOutput output, Transaction settlementTransaction) {
        this.peerGroup = peerGroup;
        this.transaction = transaction;
        this.conn = conn;
            ...
            try {
                ...
                if (payment.getSecret() != null) {
                    ...
                    if (SideConstants.RUNS_ON_SERVER) {
                        inputScript = ScriptTools.getSettlementScriptSig(channel, signature1, signature2, payment.getSecret(),
                                serverIssuedTransaction, payment.paymentToServer);
                    } else {
                        inputScript = ScriptTools.getSettlementScriptSig(channel, signature2, signature1, payment.getSecret(),
                                serverIssuedTransaction, payment.paymentToServer);
                    }
                    /*
